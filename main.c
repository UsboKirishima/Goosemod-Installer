...
int main() {
    ...
    if(client == 1) {
      path_client = "~/.config/discord/";
    } else if(client == 2) {
      path_client = "~/.config/discordptb/";
    } else if(client == 3) {
      path_client = "~/.config/discordcanary/";
    } else if(client == 4) {
      path_client = "~/.config/discorddevelopment/";
    }
    char *default = path_client;

    printf("\n%s%s%s%s\n", BCOLOR_GREEN, "Please insert your discord path (Default: ", default, ") (Leave blank to select default)\n\n");
    scanf("%s", &default);
    if(default == "") {
      default = path_client;
    }
